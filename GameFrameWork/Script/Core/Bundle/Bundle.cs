...
namespace FastBundle
{
    public class Bundle
    {
        ...
        internal void Load()
        {
	        FDebug.Log("Load " + path);
            OnLoad(); 
        }

        internal void Unload()
        {
	        FDebug.Log("Unload " + path);
            OnUnload(); 
        }
	...
	public class BundleWWW : Bundle, IEnumerator
	{
		...
		public override AssetBundle assetBundle
		{
			get
			{
				...
				return DownloadHandlerAssetBundle.GetContent(_request);
			}
		} 
		...
		protected override void OnLoad()
		{
			_request = UnityWebRequestAssetBundle.GetAssetBundle(path, version);
			if (_request == null)
			{
				error = path + " LoadFromFileAsync falied.";
			}
